...
export const MenuItem: React.FC<MenuItemProps> = ({ item, onAddToCart, onRemoveFromCart, branchId, cartItems = [] }) => {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex items-center p-4 gap-4 h-32 min-w-0">
          ...
          <div className="flex flex-col items-center justify-center min-w-[50px] flex-shrink-0 -ml-3">
            <div className="text-xl whitespace-nowrap" style={{ color: primaryColor }}>
      ...
      {/* Mobile Popup Modal */}
      {showMobilePopup && (
        <div className={`md:hidden fixed inset-0 z-50 flex items-center justify-center p-4 transition-all duration-300 ${
          isClosing ? 'bg-black bg-opacity-0 backdrop-blur-none' : 'bg-black bg-opacity-50 backdrop-blur-sm'
        }`}>
