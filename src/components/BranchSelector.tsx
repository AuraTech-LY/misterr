      ...
      <div className="flex-1 px-6 py-6 space-y-6 md:space-y-0 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-6 md:max-w-6xl md:mx-auto">
        {branches.map((branch) => (
          <button
            ...
            } ${!isOpen ? 'opacity-50' : 'shadow-2xl hover:shadow-3xl hover:brightness-110'} md:min-h-[140px] md:flex md:items-center`}
          >
            ...
            <div className="relative flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 md:w-14 md:h-14 bg-white/10 rounded-xl flex items-center justify-center backdrop-blur-sm group-hover:bg-white/20 group-hover:scale-110 transition-all duration-300">
                  <MapPin className="w-6 h-6 md:w-7 md:h-7 text-white" />
                </div>
                <div className="text-right md:text-center md:flex-1">
                  <div className="text-xl md:text-2xl font-bold mb-1 group-hover:scale-105 transition-transform duration-300">{branch.name}</div>
