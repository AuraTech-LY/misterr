          ...
          <div className="text-center mb-12">
            ...
            <div className="flex items-center justify-center gap-4 mb-8">
              <div className="relative">
                <div className="w-20 h-20 bg-gradient-to-br from-[#781220] to-[#5c0d18] rounded-2xl shadow-2xl flex items-center justify-center transform hover:scale-105 transition-all duration-300">
                  <img 
                ...
                {/* Subtle glow effect */}
                <div className="absolute inset-0 bg-[#781220] rounded-2xl blur-xl opacity-20 -z-10"></div>
              </div>
              <div className="text-right">
                <h1 className="text-4xl sm:text-6xl font-black text-gray-800 mb-2">
                  المستر
                </h1>
            ...
            {/* Status Bar */}
            <div className="inline-flex items-center gap-6 bg-white/80 backdrop-blur-sm px-6 py-4 rounded-2xl shadow-lg border border-gray-200/50">
            ...
            {!isOpen && (
              <div className="mt-4 bg-amber-50 border border-amber-200 text-amber-800 px-6 py-3 rounded-xl text-sm max-w-md mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {restaurants.map((restaurant) => (
              <div
                ...
                className="group bg-white/90 backdrop-blur-sm rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden cursor-pointer transform hover:scale-[1.02] border border-gray-200/50"
              >
                {/* Restaurant Header */}
                ...
                <div className="p-8 space-y-6">
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-gray-800 text-lg mb-2">الفروع المتاحة</h4>
                      <p className="text-gray-600 mb-3">
                        {restaurant.branches.length} {restaurant.branches.length === 1 ? 'فرع' : 'فروع'} في مواقع استراتيجية
                      </p>
                      <div className="space-y-2">
                        {restaurant.branches.map((branch) => (
                          <div key={branch.id} className="flex items-center gap-3 text-gray-700">
                            <div className={`w-2 h-2 rounded-full ${
                              restaurant.id === 'mister-crispy' ? 'bg-[#55421A]' : 'bg-[#781220]'
                            }`}></div>
                            <span className="font-medium">{branch.area}</span>
                            <span className="text-gray-500 text-sm">• {branch.deliveryTime}</span>
                          </div>
                        ))}
                      </div>
                    </div>
